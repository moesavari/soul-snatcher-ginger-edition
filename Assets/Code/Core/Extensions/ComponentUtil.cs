...
public static class ComponentUtil
{
    ...
    public static T Require<T>(this Component c) where T : Component
    {
        if (c == null)
        {
            Debug.LogWarning($"[Require<{typeof(T).Name}>] Caller component is null.");
            return null;
        }

        var t = c.GetComponent<T>();
        if (t == null)
            Debug.LogWarning($"[{c.GetType().Name}] Missing required {typeof(T).Name} on '{c.gameObject.name}'.");
        return t;
    }
    ...
    public static T RequireInChildren<T>(this Component c, bool includeInactive = false) where T : Component
    {
        ...
        if (t == null)
            Debug.LogWarning($"[{c.GetType().Name}] Missing child {typeof(T).Name} under '{c.gameObject.name}'.");
        return t;
    }
    ...
    public static T RequireInParent<T>(this Component c) where T : Component
    {
        ...
        if (t == null)
            Debug.LogWarning($"[{c.GetType().Name}] Missing parent {typeof(T).Name} for '{c.gameObject.name}'.");
        return t;
    }
}
